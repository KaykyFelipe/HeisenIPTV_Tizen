        ...
        fetch(apiUrl)
            ...
            .then(data => {
                ...
                if (isPaid) {
                    ...
                    var expText = data.expirationDate ? ' válida até ' + new Date(data.expirationDate).toLocaleDateString('pt-BR') : ' (Vitalícia)';
                    statusDesc.innerText = 'Este dispositivo possui uma licença paga' + expText + ' e está totalmente desbloqueado.';
                } else if (diffDays < 7) {
                    ...
                    statusDesc.innerText = 'Sua licença ou período de testes expirou. Adquira a renovação abaixo para desbloquear o aplicativo na TV.';
                    paymentOpts.style.display = 'block';
                }
            })
            .catch(err => {
